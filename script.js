...
function openSubTab(evt, parentTabId, planId) {
	...
	for (i = 0; i < subtabcontent.length; i++) { subtabcontent[i].classList.remove("active"); }
	...
	for (i = 0; i < subtablinks.length; i++) { subtablinks[i].classList.remove("active"); }
...
function parseFrequency(freqStr) {
	...
	switch (unit) {
		...
		case 'M': hours = value * 30.44 * 24; break;
		case 'Y': hours = value * 365.25 * 24; break;
...
function parseCsvData(csvText) {
	...
	for (let i = startIndex; i < lines.length; i++) {
		...
		if (parts.length >= 5) {
			...
			if (!isNaN(id) && name && frequency) { parsedData.push({ id, name, frequency, duration, suppressions }); }
			else { console.warn(`Skipping invalid line ${i + 1}: ${line}`); alert(`Warning: Invalid data format found on line ${i + 1}.`); }
		} else { console.warn(`Skipping line ${i + 1} due to insufficient columns: ${line}`); alert(`Warning: Insufficient columns found on line ${i + 1}.`); }
	}
	return parsedData;
}
	...
	tasks.forEach(task => {
		...
		task.explicitSuppressions.forEach(suppressorId => {
			if (taskMap.has(suppressorId)) {
				...
				if (!task.activeSuppressions.hasOwnProperty(suppressorId)) { task.activeSuppressions[suppressorId] = true; }
			}
		});
		tasks.forEach(potentialSuppressor => {
			if (task.id === potentialSuppressor.id) return;
			if (task.name === potentialSuppressor.name) {
				const freqA = task.frequencyData, freqB = potentialSuppressor.frequencyData;
				if (freqA.hours > 0 && freqB.hours > 0) {
					if (freqB.hours > freqA.hours && freqB.hours % freqA.hours === 0) {
						...
						if (!plan.allSuppressions.some(s => s.suppressedById === potentialSuppressor.id && s.suppressesId === task.id)) {
							...
							if (!task.activeSuppressions.hasOwnProperty(potentialSuppressor.id)) { task.activeSuppressions[potentialSuppressor.id] = true; }
						}
					}
				}
			}
...
function populateTaskTable(planId) {
	...
	tasks.forEach(task => {
		...
		if (task.suppressedBy.length === 0) { activationCell.textContent = 'None'; }
		else {
...
function handleSuppressionToggle(event) {
	...
	if (task) {
		...
		if (suppressionLink) { suppressionLink.active = isActive; }
		planData[planId].scheduledTasks = [];
		planData[planId].monthlySummary = {};
		clearSummaryTable(planId);
		clearChart();
...
function calculateAllSchedules() {
	...
	if (isNaN(startDate.getTime())) { alert("Invalid Start Date"); return; }
	if (avgDailyUsage <= 0) { alert("Average Daily Usage must be positive."); return; }
	let analysisEndDate = null;
	if (endDateInput.value) {
		analysisEndDate = new Date(endDateInput.value);
		if (isNaN(analysisEndDate.getTime())) { alert("Invalid End Date. Using default."); analysisEndDate = null; }
	}
	...
	['A','B','C'].forEach(planId => {
		if (planData[planId].loaded) { calculateAndRenderSummary(planId, startDate, overallMaxEndDate); }
	});
...
function calculateScheduleForPlan(planId, startDate, analysisEndDate, initialAge, avgDailyUsage) {
	...
	tasks.forEach(task => {
		...
		let nextDueDate = null, nextDueHour = null;
		const freqHours = task.frequencyData.hours;
		...
		let currentDueDate = nextDueDate, currentDueHour = nextDueHour;
	...
	scheduledTasks.forEach(taskInstance => {
		...
		for (const suppression of activeSuppressors) {
			...
			const suppressorInstance = scheduledTasks.find(supInstance =>
				...
				Math.abs(supInstance.dueDate.getTime() - taskInstance.dueDate.getTime()) < (1000 * 60 * 60 * 12)
			);
			if (suppressorInstance) { isSuppressed = true; break; }
		}
		if (!isSuppressed) { finalSchedule.push(taskInstance); }
	});
...
function renderCombinedImpactChart(analysisStart, analysisEnd) {
	...
	const datasets = [];
	const planColors = { A: 'rgba(54, 162, 235, 0.7)', B: 'rgba(255, 159, 64, 0.7)', C: 'rgba(75, 192, 192, 0.7)' };
	['A', 'B', 'C'].forEach(planId => {
		if (planSummaries[planId]) {
			...
			datasets.push({ label: `Plan ${planId} Impact (%)`, data: data, backgroundColor: planColors[planId] });
		}
	});
...
function renderImpactChart(labels, datasets) {
	...
	let maxImpact = 0;
	datasets.forEach(ds => { const maxInData = Math.max(...ds.data, 0); if (maxInData > maxImpact) maxImpact = maxInData; });
	...
	impactChart = new Chart(ctx, {
		...
		options: {
			...
			scales: { x: { title: { display: true, text: 'Month Year' } }, y: { title: { display: true, text: 'Est. Availability Impact (%)' }, beginAtZero: true, max: yAxisMax } }
		}
	});
}
...
printSummaryBtns.forEach(button => {
	button.addEventListener('click', (event) => { window.print(); });
});

saveSummaryBtns.forEach(button => {
	button.addEventListener('click', (event) => { const planId = event.target.dataset.plan; exportSummaryAsCSV(planId); });
});
...
function exportSummaryAsCSV(planId) {
	...
	rows.forEach(row => {
		if (row.cells.length > 1 && !row.cells[0].textContent.includes("No scheduled tasks")) {
			const cells = Array.from(row.querySelectorAll('td')).map(td => escapeCSV(td.textContent));
			csv += cells.join(',') + '\r\n';
		}
	});
...
Object.keys(csvInputs).forEach(planId => {
	csvInputs[planId].addEventListener('change', () => { if (csvInputs[planId].files && csvInputs[planId].files[0]) { loadCsvBtns[planId].click(); } });
});
...
Object.keys(loadCsvBtns).forEach(planId => {
	loadCsvBtns[planId].addEventListener('click', () => {
		...
		if (!file) { alert(`Please select a CSV file for Plan ${planId}.`); return; }
		...
		reader.onload = function(e) {
			...
			if (newData.length > 0) {
				...
				clearSummaryTable(planId);
				clearChart();
				if (planId === 'A') { updateDefaultEndDate(); }
...
function exportTasksAsCSV(planId) {
	if (!planData[planId]) { alert(`No data available for Plan ${planId}.`); return; }
	const tasks = planData[planId].tasks;
	if (!tasks || tasks.length === 0) { alert(`No tasks loaded for Plan ${planId} to save.`); return; }
